...
### Changed
- `barkley-vs-brown.html` rebuilt as a bridge rather than a third summary (224 → ~501 words). It now opens with the two questions each model actually answers — "why can't I make myself start?" (Barkley) and "if I can do it sometimes, why can't I do it all the time?" (Brown) — and routes to the full guides. The practitioner comparison matrix is kept, demoted below the reader-facing content and labelled as the practitioner view.
- In-body CTAs on the four public model pages changed from "Use This In Coaching" / "Explore Coaching" to "Work on this with a coach". The old copy addressed the reader as a practitioner; these pages exist to earn a prospective client's trust. `barkley-model-guide.html` had no in-body coaching CTA at all and now has one.
